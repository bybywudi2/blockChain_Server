package baiyang.blockchain.dao.impl;

import baiyang.blockchain.dao.UserDao;
import baiyang.blockchain.domain.Block;
import baiyang.blockchain.domain.ProblemBlock;
import baiyang.blockchain.domain.User;
import baiyang.blockchain.utils.XmlUtils;
...
public class UserDaoImpl implements UserDao {
	...
	/* (non-Javadoc)
	 * @see cn.itcast.dao.impl.UserDao#add(User)
	 */
	@Override
    ...
    //对于用户连接不上的情况，改变用户ip的状态.1代表活跃，0代表不活跃
    public void changeIpStatus(String ip, int status){
        try {
            Document document = XmlUtils.getBlocksDocument();
            Element e = (Element) document.selectSingleNode("//user[@ip='"+ip+"']");
            Attribute attr = e.attribute("status");
            int oldStatus = Integer.parseInt(attr.getValue());
            if(status != oldStatus){
				attr.setValue(Integer.toString(status));
				XmlUtils.write2BlocksXml(document);
			}
        } catch (Exception e) {
            throw new RuntimeException(e);
